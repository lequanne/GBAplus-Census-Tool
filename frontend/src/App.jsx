...
const UI = {
  ...
  navSimulate: { en: "Simulation", fr: "Simulation" },
    ...
    try {
      const resp = await fetchWithRetry("/api/messages", {
        ...
        body: JSON.stringify({
          ...
          system: `You are a Canadian survey methodologist assessing the census representativeness of a survey instrument. Census representativeness means how well a survey's respondent composition would reflect the actual proportions of different population groups in the Canadian census. ${lang === "fr" ? "Respond entirely in French." : "Respond entirely in English."} Respond ONLY with a valid JSON object, no markdown: { "overall_assessment": "2–3 sentence summary", "representativeness_score": 0-100, "score_rationale": "1 sentence", "group_findings": [{"group": "group name", "finding": "specific finding about representation", "risk": "High|Medium|Low"}], "recommendations": ["rec 1", "rec 2", "rec 3"] }`,
          messages: [{ role: "user", content: `Assess the census representativeness of this survey for these population groups: ${factorList}.\n\nSurvey instrument:\n\n${surveyText.slice(0, 6000)}` }],
        }),
    ...
    <div>
      ...
      <div style={{ ...card, borderLeft: `3px solid ${T.blueTxt}`, marginBottom: "1.25rem" }}>
        ...
        <p style={{ fontSize: 13, lineHeight: 1.65, color: T.muted }}>
          {lang === "fr"
            ? "La représentativité par rapport au recensement mesure si la composition des répondants d'un sondage reflète les véritables proportions des groupes de population dans le recensement canadien. Un sondage représentatif devrait, par exemple, inclure environ 5 % de répondants autochtones (la part du recensement), 27 % de personnes en situation de handicap, et ainsi de suite. Lorsque des obstacles — mode de diffusion, langue, format accessible, niveau de lecture — réduisent la participation d'un groupe, ce groupe est sous-représenté par rapport à sa part réelle au sein de la population canadienne."
            : "Census representativeness measures whether a survey's respondent composition reflects the actual proportions of population groups in the Canadian census. A representative survey should include approximately 5% Indigenous respondents (the census share), 27% persons with disabilities, and so on. When barriers — delivery mode, language, accessible format, reading level — reduce a group's participation, that group becomes under-represented relative to its true share of the Canadian population."
          }
        </p>
      </div>

      {/* Survey upload for AI assessment */}
      ...
      <textarea
        ...
        style={{ ...inputBase, width: "100%", minHeight: 140, resize: "vertical", marginBottom: "1.25rem" }}
        readOnly={false}
      />

      {/* Inline delivery settings */}
      <span style={sLabel}>{lang === "fr" ? "Paramètres de diffusion (affectent la représentativité)" : "Delivery settings (affect representativeness)"}</span>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: "1.25rem" }}>
        <button onClick={run} disabled={simLoading || activeFactors.length === 0} style={btnPrimary(simLoading || activeFactors.length === 0)}>
          {simLoading ? (lang === "fr" ? "Calcul en cours…" : "Calculating…") : "◆ " + (lang === "fr" ? "Évaluer la représentativité" : "Assess representativeness")}
        </button>
        {activeFactors.length === 0 && <p style={{ fontSize: 12, color: T.amberTxt, alignSelf: "center" }}>{lang === "fr" ? "Sélectionnez des facteurs dans l'onglet Saisie du sondage" : "Select factors in Survey input tab"}</p>}
      </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: "1.25rem", flexWrap: "wrap" }}>
            <div style={{ background: T.surface, borderRadius: 10, padding: "0.9rem 1.25rem", display: "flex", alignItems: "center", gap: 14 }}>
              <div>
                <p style={{ fontSize: 11, color: T.muted, marginBottom: 4 }}>{lang === "fr" ? "Indice de représentativité global" : "Overall representativeness index"}</p>
                <p style={{ fontSize: 28, fontWeight: 600, color: (() => { const s = indexStyle(simResult.avgIndex); return s.color; })() }}>{simResult.avgIndex}/100</p>
              </div>
              <span style={{ background: indexStyle(simResult.avgIndex).bg, color: indexStyle(simResult.avgIndex).color, fontSize: 12, fontWeight: 500, padding: "4px 12px", borderRadius: 20 }}>{indexStyle(simResult.avgIndex).label}</span>
            </div>
            <p style={{ fontSize: 11, color: T.hint, maxWidth: 340 }}>
              {lang === "fr"
                ? "100 = parfaitement représentatif du recensement. En dessous de 90 = écart notable. En dessous de 75 = sous-représentation significative."
                : "100 = perfectly census-representative. Below 90 = notable gap. Below 75 = significant underrepresentation."}
            </p>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ borderBottom: `0.5px solid ${T.border}` }}>
                  {[
                    lang === "fr" ? "Groupe (recensement)" : "Group (census)",
                    lang === "fr" ? "Part du recensement" : "Census share",
                    lang === "fr" ? "Part estimée du sondage" : "Est. survey share",
                    lang === "fr" ? "Indice" : "Index",
                    lang === "fr" ? "Statut" : "Status",
                  ].map(h => <th key={h} style={{ textAlign: "left", padding: "6px 8px", color: T.muted, fontWeight: 500 }}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {simResult.results.map(r => {
                  const s = indexStyle(r.reprIndex);
                  return (
                    <tr key={r.id} style={{ borderBottom: `0.5px solid ${T.border}` }}>
                      ...
                      <td style={{ padding: "8px 8px", color: T.text, textAlign: "right", fontWeight: 500 }}>{r.surveyPct}%</td>
                      <td style={{ padding: "8px 8px", textAlign: "right" }}>
                        <span style={{ background: s.bg, color: s.color, fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 12 }}>{r.reprIndex}</span>
                      </td>
                      <td style={{ padding: "8px 8px" }}>
                        <span style={{ fontSize: 11, color: s.color }}>{s.label}</span>
                        {r.gap < -2 && <span style={{ fontSize: 10, color: T.hint, marginLeft: 6 }}>({r.gap > 0 ? "+" : ""}{r.gap} pp)</span>}
                      </td>
                    </tr>
                  );
                })}
          ...
          {/* Barrier flags */}
          {simResult.results.some(r => r.flags.length > 0) && (
            <div style={{ ...card, marginTop: 0 }}>
              <p style={{ ...sLabel, marginBottom: 8 }}>{lang === "fr" ? "Obstacles identifiés par groupe" : "Barriers identified by group"}</p>
              {simResult.results.filter(r => r.flags.length > 0).map(r => (
                <div key={r.id} style={{ marginBottom: 10 }}>
                  ...
                  {r.flags.map((f, i) => (
                    <p key={i} style={{ fontSize: 11, color: T.muted, marginBottom: 3 }}>→ {pick(f, lang)}</p>
                  ))}
                </div>
              ))}
              <p style={{ fontSize: 10, color: T.hint, marginTop: 8 }}>
                {lang === "fr" ? "Sources : StatCan CSD 2022, CRTC mars 2023, EUTIC 2022, ESG 2016, PEICA 2022-2023, LLO partie IV" : "Sources: StatCan CSD 2022, CRTC Mar. 2023, CIUS 2022, GSS 2016, PIAAC 2022–23, OLA Part IV"}
              </p>
            </div>
          )}
        </div>
      ...
      {surveyText.trim().length > 50 && (
        <div style={{ marginTop: "1.25rem" }}>
          <div style={{ ...card, borderLeft: `3px solid ${T.green}`, marginBottom: "1rem" }}>
            ...
            <p style={{ fontSize: 13, lineHeight: 1.65, color: T.muted, marginBottom: 8 }}>
              {lang === "fr"
                ? "Cette évaluation analyse le texte de votre instrument de sondage — pas seulement les paramètres de diffusion — pour identifier comment la conception des questions, le niveau de langue, les catégories d'identité proposées et la structure générale peuvent favoriser ou défavoriser la participation de certains groupes de population par rapport à leur part réelle dans le recensement canadien."
                : "This assessment reads your survey instrument itself — not just the delivery settings — to identify how question design, reading level, identity categories offered, and overall structure may favour or disadvantage participation from specific population groups relative to their actual share in the Canadian census."
              }
            </p>
            <p style={{ fontSize: 12, lineHeight: 1.6, color: T.hint }}>
              {lang === "fr"
                ? "L'IA produit un indice de représentativité (0–100), des constats par groupe identitaire avec un niveau de risque (Élevé / Moyen / Faible), et des recommandations concrètes sur la façon dont le libellé ou la conception du sondage pourrait être ajusté pour améliorer la représentativité par rapport au recensement."
                : "The AI produces a representativeness score (0–100), per-group findings with a risk rating (High / Medium / Low), and concrete recommendations on how survey wording or design could be adjusted to improve census representativeness."
              }
            </p>
          </div>
          ...
          {aiAssessment && !aiAssessment.loading && (
            <div style={{ ...card, marginTop: "1.25rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                <p style={{ fontSize: 14, fontWeight: 500, color: T.text, flex: 1 }}>{lang === "fr" ? "Évaluation IA de la représentativité par rapport au recensement" : "AI Census Representativeness Assessment"}</p>
